...
func AssertQuarantineState(
	...
	require.Eventually(t, func() bool {
		...
		return checkCordonState(t, node, nodeName, expected.ExpectCordoned) &&
			checkTaintState(t, node, nodeName, expected.ExpectTaint) &&
			checkAnnotationStates(t, node, nodeName, expected.AnnotationChecks)
	}, EventuallyWaitTimeout, WaitInterval)
